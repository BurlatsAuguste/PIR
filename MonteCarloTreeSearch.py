    ...
    def Decision(self):
        ...
        for i in range(len(self.fils)): #choix de l'action maximisant q(s,a)
            if ((self.fils[i].valeur > self.fils[choix].valeur) and self.player == 1) or ((-self.fils[i].valeur > -self.fils[choix].valeur) and self.player == 2):
                choix = i
        self.fils[choix].Afficher_arbre()
...
import sys
while True: #on crée des parties à l'infini
    jeu = initialisation()
    afficher(jeu)
    joueur = 0
    end = fin(jeu)
    while end == 0: #si la partie n'est pas finie, elle continue
        joueur = joueur%2
        joueur += 1
        if joueur == int(sys.argv[1]): #on précise au lancement si le MCTS doit être joueur 1 ou 2
