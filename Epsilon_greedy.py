...
def Decision(Q_function, grille, epsilon, joueur):
    actions = getAction(grille)
    valeur = -2
    best = 0
    for i in range(len(actions)): #choix de la meilleure action
        new_valeur = Q_function.get_value(grille, actions[i])
        if new_valeur > valeur:
            valeur = new_valeur
            best = i 
    prob = []
    for i in range(len(actions)): #création du vecteur de probabilité 
        if i == best:
            prob.append(1-epsilon+(epsilon/len(actions)))
        else:
            prob.append(epsilon/len(actions))
    tirage = random()
    somme = 0
    choix = 0
    for i in range(len(prob)): #tirage de l'action à effectuer
        somme += prob[i]
        if tirage <= somme:
            choix = i
            break
...
import pickle
import sys
Q = MC() #initialisation de la fonction de valeur d'action
if int(sys.argv[1]) == 1: #si l'utilisateur le demande, une fonction déjà existante est chargée
    print("chargement")
    Q.charger(sys.argv[2]) 
epsilon = float(sys.argv[4]) #la valeur de epsilon est décidée par l'utilisateur
for i in range(int(sys.argv[5])): #entrainement de la fonction en jouant contre elle-même
    print(i)
    jeu = initialisation()
    joueur = 0
    episode = []
    while fin(jeu) == 0: #une partie se joue
        joueur = joueur%2
        joueur += 1
        coup = Decision(Q, jeu, epsilon, joueur)
    ...
    if gagnant == 3:
        gagnant = 0
    for step in episode: #les resusltats sont utilisés pour améliorer la fonction
        if step[0] == 2:
            Q.learn(step[1], step[2], -gagnant)
        if step[0] == 1:
            Q.learn(step[1], step[2], gagnant)
    print("bravo", gagnant)
Q.sauvegarder(sys.argv[3]) #sauvegarde de la fonction dans un fichier
while True: #l'utilisateur peut joueur contre l'IA
    joueur = 0
    jeu = initialisation()
    afficher(jeu)
    while fin(jeu) == 0:
